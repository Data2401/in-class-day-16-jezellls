...
```{r}
h1 <- ggplot(data = heights, mapping = aes(x = height)) 
h1 + geom_histogram(binwidth = 10, fill = "salmon", color = "red")
...
```{r}
h1 + geom_histogram(binwidth = 1, fill = "salmon", color = "red")
```

Smooth this out to an emperical density with `geom_density()`

```{r}
h1 + geom_histogram(binwidth = 1, fill = "salmon", color = "red") + geom_density()
```

Use a new argument in the `aes()`, `group =` to split this density by sex

```{r}
h2 <- ggplot(data = heights, mapping = aes(x = height, group = sex)) 
h2 + geom_histogram(binwidth = 1, fill = "salmon", color = "red") + geom_density()
...
h2 <- ggplot(data = heights, mapping = aes(x = height, 
                                           group = sex, 
                                           fill = sex)) 
h2 + geom_histogram(binwidth = 1) + geom_density()
```
...
h2 <- ggplot(data = heights, mapping = aes(x = height, 
                                           group = sex, 
                                           fill = sex)) 
h2 + geom_histogram(binwidth = 1) + geom_density() + facet_grid(sex ~.)
...
```{r}
h3 <- ggplot(data = heights, mapping = aes(x = height, 
                                           group = sex, 
                                           fill = sex)) + 
...
```{r}
heights %>% summarise(meanOv = mean(height),
                      median = (height))
```
...
## OR this works too 
hei <-heights %>% group_by(sex) %>%
  summarise(meanOv = mean(height),
                      medOv = median(height))
